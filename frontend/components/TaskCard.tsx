...
export function TaskCard({ task }: TaskCardProps) {
    ...
    return (
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 hover:border-indigo-500/50 transition-colors flex flex-col justify-between h-full">
            ...
            <div className="space-y-4 mt-4 pt-4 border-t border-gray-700">
                ...
                <button
                    ...
                    className="w-full py-2 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-white font-medium transition-colors"
                >
                    {task.status === 'open' ? 'View Details' : 'Unavailable'}
                </button>
            </div>
        </div>
    );
